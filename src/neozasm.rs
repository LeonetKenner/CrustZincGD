...
pub fn assemble(source: &str) -> Vec<u16> {
    ...
    for (lineno, line) in lines {
        ...
        match name {
            "mov" => {
                if args.len() != 2 {
                    panic!("'mov' needs 2 args at line {}", lineno);
                }
                a = resolve(&args[0], &labels);
                b = resolve(&args[1], &labels);
                if args[0].parse::<u16>().is_ok() {
                    f |= 1;
                }
            }
            "add" | "sub" | "and" | "or" | "xor" | "shl" | "shr" => {
                if args.len() != 3 {
                    panic!("'{}' needs 3 args at line {}", name, lineno);
                }
                a = resolve(&args[0], &labels);
                b = resolve(&args[1], &labels);
            ...
            "mul" => {
                if args.len() != 2 {
                    panic!("'mul' needs 2 args at line {}", lineno);
                }
                a = resolve(&args[0], &labels);
                b = resolve(&args[1], &labels);
                if args[0].parse::<u16>().is_ok() {
            ...
            "not" => {
                if args.len() != 2 {
                    panic!("'not' needs 2 args at line {}", lineno);
                }
                a = resolve(&args[0], &labels);
            ...
            "jmp" => {
                if args.len() != 1 {
                    panic!("'jmp' needs 1 arg at line {}", lineno);
                }
            ...
            "save" | "push" => {
                if args.len() != 1 {
                    panic!("'{}' needs 1 arg at line {}", name, lineno);
                }
                a = resolve(&args[0], &labels);
                if args[0].parse::<u16>().is_ok() {
                    f |= 1;
            ...
            "load" => {
                if args.len() != 1 {
                    panic!("'load' needs 1 arg at line {}", lineno);
                }
                c = resolve(&args[0], &labels);
                if args[0].parse::<u16>().is_ok() {
                    f |= 4;
            ...
            "pop" => {
                if args.len() != 1 {
                    panic!("'pop' needs 1 arg at line {}", lineno);
                }
                a = resolve(&args[0], &labels);
            }
            "halt" => {}
